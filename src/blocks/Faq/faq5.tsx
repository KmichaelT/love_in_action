...
const Faq5: React.FC<FaqBlock & { publicContext: PublicContextProps }> = ({ faqs, publicContext, headline, badge }) => {
  return (
          ...
          {faqs && faqs.map((faq, index) => (
            <div key={index} className="mb-8 flex gap-4">
              ...
              <div>
                ...
                {
                  faq.answer && <RichText publicContext={publicContext} content={faq.answer} overrideStyle={{ p: 'text-sm text-muted-foreground', li: 'text-sm text-muted-foreground' }} />
                }
              </div>
            </div>
          ))}
