...
const turnstileProtectedRoute = (routeHandler: (req: Request, params: ParamsPromise) => Promise<Response>) => async (req: Request, params: ParamsPromise) => {
  if (process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY) {
    ...
    if (TURNSTILE_PROTECTED_SLUGS.includes(slug[0])) {
      ...
      const turnstileResponse = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', {
        ...
        body: JSON.stringify({ secret: process.env.NEXT_PRIVATE_TURNSTILE_SECRET_KEY, response: turnstileToken }),
      })
      const data = await turnstileResponse.json()
      if (!data.success) {
        console.error('Turnstile token is invalid', data)
        return new Response(JSON.stringify({ errors: [{ message: 'Bot protection could not verify that you are a real human. Turnstile token is invalid' }], status: 400 }), { status: 400 })
      }
    }
  }
